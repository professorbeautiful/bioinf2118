...
output$thePlot = renderPlot({
  ...
  output$happyImage = 
    renderPlot({
      ...
      text(0.5, Ylevels[4] - 0.2, 'Chance "has it" if test is positive')
...
fluidPage(
  ...
  fluidRow(
    column(6, 
           radioButtons('plotChoice', label='', choiceValues=0:3,
                        choiceNames = c('decision plot', 'people plot #1',  'people plot #2', 'happy face'), 
                        inline = TRUE),
           # checkboxInput('plotToggle', value=FALSE,
           #               label='select for a "people plot"'),
           #plotOutput('thePlot')
           # conditionalPanel(condition = 'input.plotToggle === false',
           #                  plotOutput('thePlot')),
           # conditionalPanel(condition = 'input.plotToggle === true',
                            uiOutput('gerdUI') #)
           ),
