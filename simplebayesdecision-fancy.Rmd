...
dashWrap = function(s)
             HTML(paste0(
                  "<div style = 'color: rgba(0, 100, 200, 1);
                  font-weight:strong';",
                  "font-style: italic;>",
                  "∎", s
                   , "</div>"
...
observeEvent(
  input$parameterInputID,
  {
    ...
    #return()  ### sigh;  
        #HTML won't work here. Better to have the Loss labels wrap.
    setJSdefaultsAttribute = function(param, att, value, parent=FALSE) {
    ...
    #document.getElementById('specDefault').parentElement.style.color = 'green'
    for(param in paramArgNames[1:3]) {
      ...
      setJSdefaultsAttribute(param, 'color', 'blue')
      setJSdefaultsAttribute(param, 'color', 'green', parent=TRUE)
      # runjs("document.getElementById('specDefault').style.color = 'red'")
    }
    ### now, updating the rangeInputID
    updateTextInput(inputId='rangeInputID',
                    value=paramPlotRange[[f_which.varies()]])
    })
