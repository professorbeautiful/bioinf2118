...
text(axis.prob, 0, 'zero', col=probcolor, xpd=NA, adj=c(-0.2,0.6))
text(axis.prob, 1, 'one', col=probcolor, xpd=NA, adj=c(-0.2,-0.4))

temparrow = function(x,y,...)
  arrows(x0=x[1], y0=y[1], x1=x[2], y1=y[2], length=0.025, ...)
for(logodds in logoddslist[ (1:length(logoddslist)) %%5 == 0]){
  ...
  temparrow(c(axis.odds, axis.prob), 
        c(odds, odds.to.prob(odds)), col='black')
  readline("wait")
}
