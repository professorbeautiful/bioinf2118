...
server = function(input, output) {
  ...
  updateNumericInput(inputId='pr_hD', 
                     value = 1 - input$pr_dD - input$pr_iD - input$pr_noD  )
}
)
#         {
#           rV$priors =
...
# })
  ...
  output$makePosteriorSequence = renderText({
    print(c(input$dataInputID,
            input$dD, input$hD, input$iD, input$noD))
    req(!is.null(input$dataInputID))
  
    req(input$dD, input$hD, input$iD, input$noD)
    this_priorFor4 = 
      c(input$dD, input$hD, input$iD, input$noD)
 
    this_pr_pos_given_group = 
      c(.9, .1, .9, .1)
    print(paste('dataInput ', input$dataInputID))
    ...
    if(all(dataSequence %in% 1:2)) { #proceed
      ...
      (
        makePosteriorSequence(
          ...
          pr_pos_given_group = this_pr_pos_given_group,
