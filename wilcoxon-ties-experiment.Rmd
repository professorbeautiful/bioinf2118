...
```
Aha!  Now the exact P value is two orders of magnitude smaller, as it should be.
...
the computation takes `r timingFor.Xrounded3*1000` milliseconds for the "rounded to 3 digits" dataset, with its ties, and  `r timingFor.Xbrokentie*1000` milliseconds when the ties are broken by tiny arbitrary amounts.