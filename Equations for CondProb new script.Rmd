...
The prior odds is one percent, quite low. This diagnosis seems like a long shot going in the door.  The LR is high, it's 10, so getting a positive is far more likely if the person is Sick, but now multiply the odds by the LR and you've only boosted the odds up from 1% to 10%, the probability to only 9%.
...
That's the same as adding the same *log LR* every time, because the logarithm turns multiplying into adding. These little up-arrows are the same length. So let's start with odds of 0.01. The first positive test R1 brings the chance up  only to 9%.  The second one, R2, jacks it up to a half. R3 brings it to 91%, and R4 to 99%.

Just look at all that evidence mount up!
...
Here is the same probability table, but a way different answer, because SOMETHING is lurking underneath:
...
What's as bad, some patients WITH the target disease might have a variant of the disease that, in the lingo, the test is not sensitive enough. Hidden disease. "$hD$".
...
But we didn't know about hD and iD. 
The probabilities for dD and hD? We only see their sum.  Likewise for iD and noD.

The probabilities in the first table are still correct, but we cannot distinguish between dD and hD, between iD and noD?

The full story is this:

![](table_for_dD_hD_iD_noD.png)
You can see the same identical information, but now with explanations for false positives and false negatives, the hidden $hD$ and the imposters $iD$.

False results often share the same cause.  If you only knew that, you wouldn't be fooled so easily.
...
Let's just suppose the data are ${positive, positive}$. And, that conditional independence IS true for the 4 subgroups. Then
...
The two reports $\RR$ are conditionally independent... for $dD$ and for $hD$.  But not if you condition only on $+D$.

Maybe you don't even know that $+D$ contains two different types of patients.  Then you wonder: 
...
\end{aligned}
$$
Nope.  In the example, if conditional independent, then  $Pr(positive \space | \space +D) {^2} = sensitivity^2 =0.82{^2} = 0.67$. But $Pr(\RR \space | \space +D) = 0.9^2 *0.9 + 0.1^2*0.1 = 0.73$ is different, so NOT conditionally independent given $+D$.

### iD and noD = -D

On the no-disease side, 
if conditional independent, then  $Pr(positive \space | \space -D) {^2} = (1-specificity)^2 = 0.1{^2} = 0.01$. But $Pr(\RR \space | \space -D) = 1.0^2 *0.09 + 0.0^2*0.90 = 0.09$ is a lot higher, WAY different, so NOT AT ALL conditionally independent given $-D$.
