source("boxplotdatajitter.R")
source("smartBarplot-DEFINITION.R")
...
smartBarPlot(split(hotdogs$Calories, hotdogs$Type), ylim=c(0,max(hotdogs$Calories)*1.05))
title("Calories")
...
hotdogs$brand = sample(paste('Brand', LETTERS[24:26]), nrow(hotdogs), replace=TRUE)


## Using polynomial regression.
lm(data=hotdogs, Calories ~ Sodium)
anova(.Last.value)
## Higher order modeling
lm(data=hotdogs, Calories ~ poly(Sodium, degree=2))
anova(.Last.value)
lm(data=hotdogs, Calories ~ Sodium + poly(Sodium, degree=2))
anova(.Last.value)
lm(data=hotdogs, Calories ~ poly(Sodium, degree=2) + Sodium)
anova(.Last.value)
lm(data=hotdogs, Calories ~  Sodium + Sodium^2)
anova(.Last.value)
lm(data=hotdogs, Calories ~  Sodium + I(Sodium^2))
anova(.Last.value)
## Interactions between two variables.
lm(data=hotdogs, (Type=="Poultry") ~ (Calories + Sodium)^2)
anova(.Last.value)
